...
public class BoardControllerDocsTest extends RestDocsSupport {
    ...
    void createNoticeBoard() throws Exception {
        ...
        CreateBoardResponse response = CreateBoardResponse.builder()
            ...
            .createdDate(LocalDateTime.now())
            .build();
        given(boardService.createNoticeBoard(anyString(), anyLong(), any()))
            .willReturn(response);

        mockMvc.perform(
                post("/board-service/v1/schools/{schoolId}/boards/notices", 1L)
                    .content(objectMapper.writeValueAsString(request))
                    .contentType(MediaType.MULTIPART_FORM_DATA)
            )
            .andDo(print())
            .andExpect(status().isCreated())
            .andDo(document("create-notice-board",
                preprocessRequest(prettyPrint()),
                preprocessResponse(prettyPrint()),
                requestFields(
                    fieldWithPath("title").type(JsonFieldType.STRING)
    ...
    void createCommunicationBoard() throws Exception {
        ...
        CreateBoardResponse response = CreateBoardResponse.builder()
            ...
            .createdDate(LocalDateTime.now())
            .build();
        given(boardService.createCommunicationBoard(anyString(), anyLong(), any()))
            .willReturn(response);

        mockMvc.perform(
                post("/board-service/v1/schools/{schoolId}/boards/communications", 1L)
                    .content(objectMapper.writeValueAsString(request))
                    .contentType(MediaType.MULTIPART_FORM_DATA)
            )
            .andDo(print())
            .andExpect(status().isCreated())
            .andDo(document("create-communication-board",
                preprocessRequest(prettyPrint()),
                preprocessResponse(prettyPrint()),
                requestFields(
                    fieldWithPath("title").type(JsonFieldType.STRING)
